...
def main():
    ...
    for i, src in enumerate(filtered):
        dst = neg_out / f"cv_negative_{i:04d}.wav"
        dst.symlink_to(src.resolve())
    ...
    for i, src in enumerate(ambient_files):
        dst = amb_out / f"ambient_{i:04d}.wav"
        dst.symlink_to(src.resolve())
