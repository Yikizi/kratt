...
The results show that no current model or combination of models satisfies all targets simultaneously: low streaming FAPH, high recall on real speakers, low hard-negative FPR, and accurate phrase-structure distinction. The main contribution is therefore a reproducible KWS evaluation protocol and an empirical description of the trade-offs that arise in small-resource-language local wake-word development, rather than a production-grade voice assistant or a user-study result. User testing is treated as future system-level validation for a larger assistant, not as a prerequisite for this machine-learning-focused thesis.
