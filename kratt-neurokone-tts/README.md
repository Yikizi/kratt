...
This add-on runs the TartuNLP neural TTS worker inside the Home Assistant add-on container. On first start it downloads the multispeaker model into `/data/models`; after that, synthesis runs locally.
...
Experimental research-prototype packaging for the Kratt thesis project. After the first-start model download, synthesis text stays on the local Home Assistant host/container.
