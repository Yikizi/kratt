...
#define NUM_MICS 3         // MIC1, MIC2, MIC3
...
static esp_err_t init_sd_card(void) {
  esp_vfs_fat_sdmmc_mount_config_t mount_config = {
      ...
      .max_files = 5, // 3 WAV files + headroom
  };
...
static button_t read_button(adc_oneshot_unit_handle_t adc) {
  int raw;
  if (adc_oneshot_read(adc, BUTTON_ADC_CHANNEL, &raw) != ESP_OK) return BTN_NONE;
  if (raw >= BTN_NONE_MIN) return BTN_NONE;
  if (raw >= BTN_REC_MIN && raw <= BTN_REC_MAX) return BTN_REC;
  if (raw >= BTN_MUTE_MIN && raw <= BTN_MUTE_MAX) return BTN_MUTE;
  return BTN_NONE;
}
...
void app_main(void) {
  ...
  // I2S read buffer: TDM_SLOTS channels interleaved
  int16_t i2s_buf[I2S_BUF_SAMPLES * TDM_SLOTS];
  // Per-mic mono buffers for deinterleaving
  ...
  while (true) {
    ...
    if (btn_pressed && btn == BTN_REC && !recording) {
      ...
      for (int m = 0; m < NUM_MICS; m++) {
        char path[64];
        snprintf(path, sizeof(path), MOUNT_POINT "/rec_%04d_mic%d.wav",
                 rec_num, m + 1);
  ...
  skip:
    ...
    if (recording) {
      ...
      // Debug: print first 16 int16_t values to see actual TDM layout
      if (!debug_printed) {
        ESP_LOGI(TAG, "I2S buf (%d bytes). First 16 values:", (int)bytes_read);
        for (int d = 0; d < 16 && d < (int)(bytes_read / sizeof(int16_t)); d++) {
          ESP_LOGI(TAG, "  [%2d] = %6d (0x%04X)", d, i2s_buf[d],
                   (uint16_t)i2s_buf[d]);
        }
        debug_printed = true;
      }

      // Deinterleave: TDM data is [S0,S1,S2,S3, S0,S1,S2,S3, ...]
      int samples = bytes_read / (TDM_SLOTS * sizeof(int16_t));
      for (int i = 0; i < samples; i++) {
        for (int m = 0; m < NUM_MICS; m++) {
          mic_buf[m][i] = i2s_buf[i * TDM_SLOTS + m];
        }
      }
