...
SAMPLE_RATE = 16000
DURATION = 5  # seconds
WAKE_WORD_DURATION = 2  # seconds for wake word detection
LAMP_IP = "172.20.10.3"
...
Estonian colors: punane=red, sinine=blue, roheline=green, kollane=yellow, oranž=orange, lilla=purple, roosa=pink, valge=white, hall=gray, pruun=brown.
Modifiers: hele=light, tume=dark.

IMPORTANT: If the user says "must" (black), treat it as turn OFF command.

Estonian commands:
- "sisse" or "peal" or "põlema" = turn on
- "must" (black) or "välja" or "kinni" or "ära" = turn off"""
...
def has_speech_energy(audio, threshold=0.02):
    """Quick check if audio has enough energy to contain speech"""
    rms = np.sqrt(np.mean(audio**2))
    return rms > threshold


def listen_for_wake_word():
    """Listen for wake word in short bursts with energy detection"""
    # Record short audio clip
    audio = sd.rec(
        int(WAKE_WORD_DURATION * SAMPLE_RATE),
    ...
    # Quick energy check - skip transcription if too quiet
    ...
    # Transcribe only if there's enough audio energy
    try:
...
def record_and_transcribe():
    ...
    print("   Say your command in Estonian!")
    ...
    # Transcribe with optimized Transformers pipeline
    print("🔄 Transcribing...")
    result = pipe(temp_file)
...
async def main():
    ...
    # Start web server for phone trigger
    local_ip = socket.gethostbyname(socket.gethostname())
    port = 5555
    trigger_queue = web_trigger.start_server(host="0.0.0.0", port=port)

    print("\n" + "=" * 50)
    print("📱 Web Control Interface Started!")
    print(f"   Open on your phone: http://{local_ip}:{port}")
    print("   (Make sure phone is on same WiFi network)")
    print("=" * 50)
    print("\n✅ Ready! Tap the button on your phone to record")
    print("   Press Ctrl+C to quit\n")

    try:
        while True:
            # Wait for phone trigger
            print("📱 Waiting for phone tap...", end="\r")
            web_trigger.wait_for_trigger(trigger_queue)
